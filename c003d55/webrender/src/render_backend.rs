    ...
    fn build_scene(&mut self) {
        ...
        for (id, webgl_context) in &self.webgl_contexts {
            if self.dirty_webgl_contexts.contains(&id) {
                ...
                let fence = webgl_context.gl().fence_sync(gl::SYNC_GPU_COMMANDS_COMPLETE, 0);
                webgl_context.gl().client_wait_sync(fence, gl::SYNC_FLUSH_COMMANDS_BIT, gl::TIMEOUT_IGNORED);
                webgl_context.gl().delete_sync(fence);
